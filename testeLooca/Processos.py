...
def inserirProcesso(processoProcurado):
    ...
    #SQL LOCAL
        ...
        bdLocal_cursor.execute(query, (processoProcurado['pid'],processoProcurado['titulo'],8,8,1))
        bdLocal_cursor.close()
        mysql_cnx.commit()
    # SQL SERVER
    else:
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query,(processoProcurado['pid'],processoProcurado['titulo'],8,8,1))
...
def deletarProcesso(processoProcurado):
    ...
    #SQL LOCAL
        bdLocal_cursor = mysql_cnx.cursor()
        bdLocal_cursor.execute(query, (8, processoProcurado['pid'],processoProcurado['titulo']),)
        bdLocal_cursor.close()
        mysql_cnx.commit()
    else:
    # SQL SERVER
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query, (8, processoProcurado['pid'],processoProcurado['titulo']),)
        bdServer_cursor.close()
        sql_server_cnx.commit()
...
def obterDadosGrafico(qtd, cpu, ram):
    ...
    #SQL LOCAL
        bdLocal_cursor = mysql_cnx.cursor()
        bdLocal_cursor.execute(query, (qtd,cpu,ram, 8,1),)
        bdLocal_cursor.close()
        mysql_cnx.commit()
    else:
    # SQL SERVER
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query, (qtd,cpu,ram, 8,1),)
        bdServer_cursor.close()
        sql_server_cnx.commit()
