...
object Python {
    ...
    fun criarScript(tempo:Int, idMaquina:Int, idEmpresa:Int):String {
...
def inserirProcesso(processoProcurado):
    ...
    #SQL LOCAL
        ...
        bdLocal_cursor.execute(query, (processoProcurado['pid'],processoProcurado['titulo'],8,$idMaquina,$idEmpresa))
        bdLocal_cursor.close()
        mysql_cnx.commit()
    # SQL SERVER
    else:
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query,(processoProcurado['pid'],processoProcurado['titulo'],8,$idMaquina,$idEmpresa))
...
def deletarProcesso(processoProcurado):
    ...
    #SQL LOCAL
        bdLocal_cursor = mysql_cnx.cursor()
        bdLocal_cursor.execute(query, ($idMaquina, processoProcurado['pid'],processoProcurado['titulo']),)
        bdLocal_cursor.close()
        mysql_cnx.commit()
    else:
    # SQL SERVER
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query, ($idMaquina, processoProcurado['pid'],processoProcurado['titulo']),)
        bdServer_cursor.close()
        sql_server_cnx.commit()
...
def obterDadosGrafico(qtd, cpu, ram):
    ...
    #SQL LOCAL
        bdLocal_cursor = mysql_cnx.cursor()
        bdLocal_cursor.execute(query, (qtd,cpu,ram, $idMaquina,$idEmpresa),)
        bdLocal_cursor.close()
        mysql_cnx.commit()
    else:
    # SQL SERVER
        bdServer_cursor = sql_server_cnx.cursor()
        bdServer_cursor.execute(query, (qtd,cpu,ram, $idMaquina,$idEmpresa),)
        bdServer_cursor.close()
        sql_server_cnx.commit()
if __name__ == "__main__":
    ...
    """.trimIndent()
        ...
        val diretorioDoProjeto = System.getProperty("user.dir")

        val nomeArquivoPy = "Processos.py"

        val arquivo = File(diretorioDoProjeto, nomeArquivoPy)
        arquivo.writeText(codigoPython)

        val caminhoAbsoluto = arquivo.absolutePath
        //println("Arquivo criado em: $caminhoAbsoluto")
